...
export function Dashboard({ onStartInterview }: DashboardProps) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(fileExtension)) {
      setUploadError('Please upload PDF, DOCX, or TXT files only');
      return;
    }
  ...
  <div className="space-y-2">
    ...
    <div className="bg-yellow-50 p-2 border border-yellow-200 rounded">
      <div className="text-xs font-medium mb-1">Debug Data:</div>
      <pre className="text-xs overflow-auto">
        {JSON.stringify(uploadResult.extracted_skills, null, 2)}
      </pre>
    </div>
    
    {/* Skills Preview - UPDATED CODE */}
    {uploadResult.extracted_skills.skills_found.length > 0 && (
      <div className="space-y-2">
        <div className="text-xs font-medium text-gray-700">
          Skills found: {uploadResult.extracted_skills.total_skills}
        </div>
        <div className="flex flex-wrap gap-1">
          {uploadResult.extracted_skills.skills_found.slice(0, 4).map((skill, index) => (
            <Badge key={index} variant="secondary" className="text-xs">
              {skill.Category || skill.category || skill.skill}
            </Badge>
          ))}
          {uploadResult.extracted_skills.total_skills > 4 && (
            <Badge variant="outline" className="text-xs">
              +{uploadResult.extracted_skills.total_skills - 4} more
            </Badge>
          )}
        </div>
      </div>
